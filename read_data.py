...
def build_full_dataset(name, img_shape, include_bad_quality_images):
    idx_valid = np.array(
        [i for i in age.keys() if
         not bad_age[i] and not bad_sex[i] and (not bad_quality[i] or include_bad_quality_images)])
    dataset = dict()
    ...
    if dataset_file.is_file() and ecv_file.is_file():
        ...
        folds = stratified_cv_split_ttv_age_and_sex(np.array([age[i] for i in idx_valid]),  # Vector de idades
                                                    np.array([sex[i] for i in idx_valid]),  # Vector de sexos
...
def build_dataset_age_range(full_dataset, min_age, max_age, dataset_name, include_bad_quality_images):
    ...
    if ecv_file.is_file():
        ...
        folds = stratified_cv_split_ttv_age_and_sex(age=np.array([age[i] for i in idx_valid]),
                                                    sex=np.array([age[i] for i in idx_valid]),
                                                    k=8,
...
full_dataset_w_bad_quality = build_full_dataset(name="all", img_shape=RESIZED_SHAPE,
                                                include_bad_quality_images=inc_bad_qual)
build_dataset_age_range(full_dataset_w_bad_quality,
                        min_age=0 * 365, max_age=40 * 365,
                        dataset_name="40", include_bad_quality_images=inc_bad_qual)
